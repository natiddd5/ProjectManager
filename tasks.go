...
func (s *TasksService) handleGetTask(w http.ResponseWriter, r *http.Request) {
	...
	if err == sql.ErrNoRows {
		WriteJSON(w, http.StatusNotFound, ErrorResponse{"Task does not exist."})
		return
	} else {
		WriteJSON(w, http.StatusInternalServerError, ErrorResponse{"Error while trying to retrieve the task"})
		return
	}
